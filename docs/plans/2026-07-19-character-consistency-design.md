...
現況：`imageShotHandler`（src/lib/workers/handlers/media-handlers.ts）生每個鏡頭圖時，**只傳文字 prompt，零參考圖**。角色喺唔同鏡頭獨立重畫 → 樣唔一致。短劇最致命嘅質素問題。

## 1. 三家共識（= 正確做法）

全部用 **reference-image 條件生成（img2img）**，唔用 seed / LoRA / embedding：

- 每角色一張 canonical「英雄圖」（正面、中性背景、素顏），重用去所有鏡頭。
- 每鏡收集有序參考陣列 `[出場角色圖…, 場景圖]`，只收該鏡真正出現嘅資產。
- 全部壓縮成 base64 data-URI（供應商無關；huobao 768px q68、上限 6）。
- prompt 用**數字**唔用名綁定參考（`图片N` / `@图N`）+ 圖例 legend。
- 明文連續性條款：同一面孔/髮型/服飾 across ALL shots；只准改構圖/角度/動作/表情；唔准加參考圖以外嘅新角色。

**fal 關鍵**（我哋 stack）：nano-banana 有參考圖時要用 `fal-ai/nano-banana/edit`（或 `nano-banana-pro/edit`）endpoint，參考圖放 body 的 `image_urls: string[]`；無參考時走 base（text-to-image）。
...
2. **fal adapter 加 reference 支援** `src/lib/ai/adapters/fal.ts`
   ...
   - `generateImage` req 加 `referenceMediaIds?: string[]`，經 outbound 正規化後傳落 adapter。
...
5. **光影 offload**：角色鎖定圖生成時 prompt 保持中性光（`generateAssetImages` 已用 style prefix；確認唔加場景光）；鏡頭光影靠場景圖 + 鏡頭 prompt。

### M2（增值）

1. **自參考換裝** — `CharacterAppearance` 概念（v1 用 Character 單圖；M2 加 appearances[]）。canonical（index 0）純文字生；其他 look 用 canonical 圖做 reference img2img → 換衫唔換面。
2. **四視圖 turnaround 表** — 角色 canonical 生成改用「正/側/背 + 特寫」多視圖 sheet（Toonflow art_character.md 規則）→ 最大化參考面、跨角度更穩。用 `CHARACTER_ASSET_IMAGE_RATIO` 較闊比例。
3. **幀鏈**（huobao）— 若做首尾幀，尾幀把首幀當參考。
4. **空間一致性**（Toonflow）— storyboard 加 `orientation` + 3×3 grid + 180° 軸線欄，prompt 鎖每角色固定畫面側與朝向。

### 刻意唔抄

- **seed 操作** — 三家都話唔靠 seed（ali 的 seed trick 只係無參考圖時的退化方案）。
- props 參考圖 — 短劇 props 少，v1 靠文字。

## 3. Schema 影響

- M1：**零 schema 改動**（用現有 `Character.lockedImageMediaId` / `Location.lockedImageMediaId`）。
- M2 自參考換裝：加 `Character.appearances Json`（或新 model），非必須，痛咗先做。
...
- E2E `scripts/smoke-consistency.ts`：真 fal，鎖定角色圖 → 生 2-3 個含同一角色的鏡頭圖 → 人眼/檔案存在斷言（自動化只能斷言 image_urls 有傳、endpoint 對；一致性靠人睇）。
- **收貨**：同一角色喺多個鏡頭生出嚟，面孔/髮型/服飾肉眼一致（對比改動前每鏡各異）。
