...
export function DailyCostChart({ rows }: { rows: UsageRowView[] }) {
  ...
  const max = Math.max(...rows.map((r) => r.estCostUsd), 0);

  return (
    <div className="barchart" role="img" aria-label="按日成本圖">
      {rows.map((r) => {
        const h = max > 0 ? (r.estCostUsd / max) * 100 : 0;
        const mmdd = r.key.slice(5); // MM-DD
        return (
          <div key={r.key} className="bar-col" title={`${r.key} · ${usd(r.estCostUsd)} · ${r.calls} calls`}>
