...
import { Loader2, Upload, UserRound, Volume2 } from "lucide-react";
...
function CastRow({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-lg border p-2">
      ...
      {refAudioUrl && <audio src={refAudioUrl} controls preload="none" className="h-8" />}
      {row.note && <span className="text-xs text-muted-foreground">{row.note}</span>}
      {save.err && <span className="text-xs text-destructive">{save.err}</span>}
    </div>
  );
}
