// The ✋ asset review gate for props — same pattern as locations/[id]/lock.
...
export const POST = withAuth(
  async ({ userId, params, req }) => {
    ...
    if (body.unlock) {
      await prisma.prop.update({ where: { id: row.id }, data: { locked: false } });
    ...
    await prisma.prop.update({ where: { id: row.id }, data: { lockedImageMediaId: body.mediaId, locked: true } });
    // Props are project-level (shared across episodes) — kick every autorun
